...
function normalizePartnerPayload(data = {}) {
  ...
  if (data.company_type === "person" || data.company_type === "company") clean.company_type = data.company_type;
...
app.get("/api/odoo/companies/search", async (req, res) => {
  try {
    ...
    return res.json({ ok: true, companies: companies || [] });
  } catch (e) {
    console.error(e);
    return res.status(500).json({ ok: false, message: String(e?.message || e) });
  }
...
app.post("/api/odoo/partners/create-or-update", async (req, res) => {
  try {
    ...
    if (mode === "create") {
      ...
      if (!data.name) {
        data.name = data.email.split("@")[0] || "Contacto";
      }
