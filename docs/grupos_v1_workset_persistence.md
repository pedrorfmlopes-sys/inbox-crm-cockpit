...
- No client, as chamadas HTTP de workset vivem em `client/src/modules/crm/groups-v1/storage/worksetApi.ts`.
- `saveWorkset.ts` e `loadWorkset.ts` nao importam `client/src/api.ts`, para manter storage/worksets fora do hub global de arranque.
...
- Nao ha dependencia runtime de `groups-v1/storage/*` para `client/src/api.ts`; a fronteira HTTP de workset fica isolada em `worksetApi.ts`.
